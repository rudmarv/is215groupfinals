<x-app-layout>
    ...
    <script>
        function checkURLink(url) {
            function fetchURL() {
                fetch(url)
                .then(response => {
                    // Check if response status is ok
                    if (response.ok) {
                    console.log(`URL is ready - Status: ${response.status}`);
                    } else {
                    // If response status is not ok, log a message without displaying error
                    console.log(`Failed to fetch URL - Status: ${response.status}`);
                    }
                })
                .catch(error => {
                    // Silently handle any errors without displaying them
                });
            }

            // Call fetchURL initially and then set it to be called recursively every 5 seconds
            function checkRepeatedly() {
                fetchURL();
                setTimeout(checkRepeatedly, 5000);
            }

            checkRepeatedly();
            }

            // Usage example: Check the URL every 5 seconds
            checkURL('https://is215finals.s3.amazonaws.com/articles/1714672084ss1714624186Forest.jpg-article.txt');
        ...
        $('#uploadBtn').click(function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    checkURLink('https://is215finals.s3.amazonaws.com/articles/'+response+'-article.txt',10);
                },
